const ENCRYPTION_ENABLED = process.env.NEXT_PUBLIC_API_PAYLOAD_ENCRYPTION_ENABLED === 'true';
const ENCRYPTION_KEY_HEX = process.env.NEXT_PUBLIC_API_ENCRYPTION_KEY || '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef';
...
export async function apiClient(endpoint: string, options: ApiOptions = {}) {
  ...
  if (ENCRYPTION_ENABLED && jsonResponse && typeof jsonResponse.data === 'string') {
    try {
      jsonResponse = await decryptPayload(jsonResponse.data);
    } catch (e) {
  ...
  if (!response.ok) {
    ...
    if (jsonResponse) {
      errorMessage = jsonResponse.error || jsonResponse.message || JSON.stringify(jsonResponse);
    }
    
    // Fallback for empty/missing error messages on 401
    if (response.status === 401 && (!errorMessage || errorMessage === 'Unauthorized' || errorMessage.includes('{"data"'))) {
...
export const api = {
  get: (endpoint: string, options?: ApiOptions) => apiClient(endpoint, { ...options, method: 'GET' }),
  post: (endpoint: string, data: any, options?: ApiOptions) => apiClient(endpoint, { ...options, method: 'POST', data }),
  patch: (endpoint: string, data: any, options?: ApiOptions) => apiClient(endpoint, { ...options, method: 'PATCH', data }),
  delete: (endpoint: string, options?: ApiOptions) => apiClient(endpoint, { ...options, method: 'DELETE' }),
};
